...
#
# Cosmos3-Edge action-policy SFT on the SO-ARM101 pick-place dataset (experiment
# `action_policy_so101_edge`). Single-GPU by default (NPROC_PER_NODE=1).
#
...
#   NPROC_PER_NODE=1 bash examples/launch_sft_action_policy_so101_edge.sh
#
# Smoke test:
#   EXTRA_TAIL_OVERRIDES="job.wandb_mode=disabled trainer.max_iter=10 checkpoint.save_iter=10" \
#     NPROC_PER_NODE=1 bash examples/launch_sft_action_policy_so101_edge.sh

TOML_FILE="examples/toml/sft_config/action_policy_so101_edge.toml"
